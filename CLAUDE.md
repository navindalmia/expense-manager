...
- **Regression pack (cumulative, hook-enforced)** — every fix adds one `issue-<N>-<slug>.test.ts` under `backend/src/__tests__/regression/` or `frontend/src/__tests__/regression/` (`.spec.ts` under `e2e/regression/` for E2E, not run at pre-commit). `.githooks/pre-commit` always runs both suites; `.githooks/commit-msg` blocks `fix(...)`/`fix:` commits with no staged regression file unless a `Regression-Exempt: <reason>` trailer is present. Enable once per clone with `npm run hooks:install`. Logic: `scripts/regression-gate.js` (tests: `npm run test:gate`). The git hooks (auto-installed by root `npm install` via `prepare`) plus the CI `regression-gate` job (which also blocks `--no-verify`) are the universal gate; the `.claude/hooks/*` gates apply only inside Claude sessions. One newly added `issue-<N>-*.test.ts` with a real test under a backend/frontend regression dir satisfies both the fix-commit rule here and the Claude quality gate's regression-test rule. Exemptions are separate trailers: `Regression-Exempt:` (this gate, also needed to delete regression tests) vs `Test-Exempt:`/`E2E-Exempt:` (Claude gates); one does not imply the other. `.skip`/`.todo`/`.only`/`.fails` are banned in regression dirs, a qualifying file needs at least one real test call and one assertion, and deleting or weakening (fewer tests/assertions) an existing regression test needs `Regression-Exempt:` on that commit. **The gate cannot prove a test is red-before-green or actually exercises the fix** — that is enforced by `/ce-code-review` and the PR description, not mechanically. Accepted gaps: message-only reword / `--allow-empty` fix commits pass locally (CI `pr` mode is the backstop), and `--no-verify` is only closed by CI plus branch protection.
