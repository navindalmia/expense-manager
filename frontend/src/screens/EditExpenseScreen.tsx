...
export default function EditExpenseScreen({ navigation, route }: EditExpenseScreenProps) {
  ...
  useEffect(() => {
    if (expense) {
      ...
      if (expense.splitWith?.length > 0) {
        expense.splitWith.forEach(user => addMember(user.id));
        if (expense.splitType === 'PERCENTAGE' && expense.splitPercentage) {
          ...
          expense.splitWith.forEach((user, idx) => {
            if (expense.splitAmount?.[idx]) updateAmount(user.id, expense.splitAmount[idx].toString());
          });
        }
  ...
  const handleUpdate = useCallback(async () => {
    ...
    setSubmitting(true);
    try {
      const payload: any = { title: formState.title.trim(), amount: parseFloat(formState.amount), categoryId: formState.category, paidById: formState.paidById, expenseDate: formState.date, notes: formState.notes.trim() || undefined, ...getSplitPayload() };
      await updateExpense(expenseId, payload);
      logger.info('Expense updated', { expenseId, title: formState.title });
      navigation.goBack();
    } catch (err) {
      const msg = getErrorMessage(err);
      logger.error('Update failed', err, { expenseId });
