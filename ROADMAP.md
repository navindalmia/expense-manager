...
| **Intelligence layer (infra) — 🟡 IN PROGRESS, 4/10 units shipped 2026-09-05** | Themes (reusable, editable, links groups across time), user-extensible Categories, cross-group Labels + Manage Labels screen, expense-title autocomplete/prefill (global, fuzzy-match past expenses), keyword-dictionary category auto-suggestion for new titles. Explicitly infra-first — no dashboard/Q&A UI built by this item. Plan (now implementation-ready, reviewed via `/ce-doc-review`): [`docs/plans/2026-08-31-001-feat-intelligence-layer-themes-labels-autocomplete-plan.md`](docs/plans/2026-08-31-001-feat-intelligence-layer-themes-labels-autocomplete-plan.md). Blocker (CI/Maestro plan) resolved — no longer blocked. **Branch `feat/intelligence-layer-themes-labels-autocomplete`, not yet merged.** U1 (schema), U2 (Category CRUD — also fixed a real pre-existing auth gap in `categoryRoutes.ts`), U3 (Theme CRUD), U4 (Label CRUD + totals) done, tested, live-verified against local Postgres. **Next session: U5 (autocomplete search), U6 (keyword-dictionary suggestion + audit log) — both backend — then U7–U10 (frontend: shared dropdown component, screen wiring, autocomplete UI, Manage Labels screen).** |
