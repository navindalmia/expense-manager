...
- [ ] **Logged 2026-08-31, blocked Dependabot PRs need real fixes:**
  - **PRs #24/#22/#20** (vitest 1→4, vite 5→8, `@vitejs/plugin-react` 4→6 — all frontend-test failures, same root cause): Vitest 4's `vi.fn<T>` changed from a two-generic form (`vi.fn<[args], returnType>`) to a single function-type generic. Breaks `tsc` in `frontend/src/components/__tests__/AddMemberModal.test.tsx` and `frontend/src/screens/__tests__/EditExpenseScreen.test.tsx` (3 call sites each). Small, mechanical fix — update the `vi.fn` generic syntax at those 6 call sites, then these 3 PRs should go green together (bundle the fix, don't merge them individually).
