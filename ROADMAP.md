...
| **Intelligence layer (infra) — 🟡 ALL 10 UNITS IMPLEMENTED 2026-09-14, NOT YET MERGED** | Themes (reusable, editable, links groups across time), user-extensible Categories, cross-group Labels + Manage Labels screen, expense-title autocomplete/prefill (global, fuzzy-match past expenses), keyword-dictionary category auto-suggestion for new titles. Explicitly infra-first — no dashboard/Q&A UI built by this item. Plan: [`docs/plans/2026-08-31-001-feat-intelligence-layer-themes-labels-autocomplete-plan.md`](docs/plans/2026-08-31-001-feat-intelligence-layer-themes-labels-autocomplete-plan.md). **Branch `feat/intelligence-layer-themes-labels-autocomplete`, not yet merged.** U1-U6 (backend: schema, Category/Theme/Label CRUD, autocomplete search, keyword-dictionary suggestion) and U7-U10 (frontend: shared `TypeAheadDropdown`, Theme/Category/Label screen wiring, title-autocomplete UI, Manage Labels screen) all done, `tsc` clean both sides, full backend (24 suites/409 tests) + frontend (17 suites/153 tests) unit suites green. **Not yet done, before this can be called complete:** (1) real regression coverage — every test so far mocks Prisma/`axios`/service calls, zero E2E/Maestro coverage exists for any of this feature's UI (see `PROJECT_MEMORY`/AI-workflow items 37-39, a standing rule going forward, not just this feature); (2) manual smoke test against a live backend+DB was never finished; (3) PR not opened, branch not merged, CI not run on it. |
