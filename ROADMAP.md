...
- [x] **Fixed 2026-09-05** (PR #31, merged): vitest 1→4, vite 5→8, `@vitejs/plugin-react` 4→6 bumped together, with the `vi.fn<T>` generic-syntax fix (two-generic tuple → single function-type generic) at the 6 call sites in `AddMemberModal.test.tsx`/`EditExpenseScreen.test.tsx` that the bump broke. Dependabot PRs #24/#22/#20 closed as superseded — same version bump already landed.
- [ ] **Logged 2026-08-31, still blocked:**
  - **PR #18** (Prisma 6→7, backend-test failure): Prisma 7 no longer exports `PrismaClient`/`SplitType`/`Currency`/`Prisma` namespace from `@prisma/client` the old way (moved to a generated-client-only export model). Breaks `tsc` across most of `backend/src/services/`, `controllers/`, `schemas/`, `prisma.ts`, `seed.ts` — a real migration, not a one-line fix. Needs its own planned pass (`/ce-plan`), not a rushed dependency-bump merge.
...
| **Intelligence layer (infra) — 🟡 IN PROGRESS, 6/10 units shipped 2026-09-07** | Themes (reusable, editable, links groups across time), user-extensible Categories, cross-group Labels + Manage Labels screen, expense-title autocomplete/prefill (global, fuzzy-match past expenses), keyword-dictionary category auto-suggestion for new titles. Explicitly infra-first — no dashboard/Q&A UI built by this item. Plan (now implementation-ready, reviewed via `/ce-doc-review`): [`docs/plans/2026-08-31-001-feat-intelligence-layer-themes-labels-autocomplete-plan.md`](docs/plans/2026-08-31-001-feat-intelligence-layer-themes-labels-autocomplete-plan.md). Blocker (CI/Maestro plan) resolved — no longer blocked. **Branch `feat/intelligence-layer-themes-labels-autocomplete`, not yet merged.** U1 (schema), U2 (Category CRUD — also fixed a real pre-existing auth gap in `categoryRoutes.ts`), U3 (Theme CRUD), U4 (Label CRUD + totals), U5 (autocomplete search), U6 (keyword-dictionary suggestion + audit log) done, tested (tsc clean, full backend suite green). **Next session: U7–U10 (frontend: shared type-ahead dropdown component, screen wiring for Theme/Category/Label, autocomplete + suggestion UI, Manage Labels screen).** |
