...
- ✅ **New features ship with a regression test, not just bug fixes** — a `feat(...)` commit needs a test the same way a `fix(...)` commit does; functional E2E (above) proves a UI feature works, but backend-only or logic-level features still need their own test. Added 2026-09-19, mechanically enforced by `.claude/hooks/pre-commit-quality-gate.js`'s Rule 2 (broadened from `fix(`-only).
- ✅ **UI changes have a visual-regression (screenshot) baseline where feasible** — functional E2E/Maestro assertions prove a feature *works*, not that it *looks* right; an unintended layout/color/spacing change can pass every functional check. This repo's Maestro `assertScreenshot` baselines (`maestro-flows/visual/*.yaml`) are the mechanism, gated by `.claude/hooks/pre-commit-quality-gate.js`'s Rule 3. Deliberately an exempt-trailer (`Visual-Regression-Exempt: <reason>`) rule rather than a hard block, since generating/verifying a baseline needs Android emulator access most dev environments (including cloud sandbox sessions) don't have — added 2026-09-19 after a session shipped real UI changes (#47's delete-group button, #50/#51's currency picker) with full functional coverage but zero visual-regression coverage.
